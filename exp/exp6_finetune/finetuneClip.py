        ...
        for images, list_of_captions in tqdm(dataloader, desc=f"Epoch {epoch+1}"):
            ...
            for img_idx, captions in enumerate(list_of_captions):
                for cap in captions:
                    ...
                    text_to_image_indices.append(img_idx)
            ...
            sim_i2t = sim_t2i.T
            labels_t2i = torch.tensor(text_to_image_indices, device=device)
            ...
            topk_t2i = sim_t2i.topk(5, dim=1).indices
            recall_t2i = (topk_t2i == labels_t2i.unsqueeze(1)).any(dim=1).float().mean()
...
def evaluate_mcq(model, preprocess, tokenizer, csv_path, device='cuda', max_len=77):
    ...
    for i, row in tqdm(df.iterrows(), total=len(df), desc="Evaluating MCQ"):
        ...
        text_features = F.normalize(text_features, dim=-1)
        sims = (image_feature @ text_features.T).squeeze(0)
    ...
    with torch.no_grad():
        for idx, row in tqdm(df.iterrows(), total=len(df)):
            ...
            try:
                ...
                continue
            pos_caption = eval(row['captions'])[0]
            neg_caption = eval(row['n_captions'])[0]
...
if __name__ == "__main__":
    ...
    model.to(device)
    evaluate_mcq(model, preprocess, tokenizer, 'COCO_val_mcq_llama3.1_rephrased.csv', device, max_len=max_len)
    evaluate_ccneg(model, preprocess, tokenizer, 'ccneg_converted.csv', device)
